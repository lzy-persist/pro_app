package org.nacao.app.view;

import org.nacao.app.R;
import org.nacao.app.view.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener {
	...
	public interface OnPullDownListener {
		/**刷新事件接口  这里要注意的是获取更多完 要关闭 刷新的进度条

RefreshComplete()**/
		void onRefresh();
		/**刷新事件接口  这里要注意的是获取更多完 要关闭 更多的进度条 
...
notifyDidMore()**/
	...
	public void enableAutoFetchMore(boolean enable, int index) {
		if (enable) {
			...
			mFooterTextView.setText("");//信息不自动收取  加载完毕
			mFooterLoadingView.setVisibility(View.GONE);
		}
		mEnableAutoFetchMore = enable;
	}
   public void refushOVer(){
	     mIsFetchMoreing = false;
		 mFooterTextView.setText("更多信息");
		 mFooterLoadingView.setVisibility(View.GONE);
   }
   public void showInfo(){
	...
	private void initHeaderViewAndFooterViewAndListView(Context context) {
		...
		mListView.setDivider(getContext().getResources().getDrawable(R.drawable.green_divider));
