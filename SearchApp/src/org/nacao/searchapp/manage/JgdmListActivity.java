...
import org.nacao.searchapp.adater.JgdmListAadpter;
...
public class JgdmListActivity extends Activity implements
	...
	private TextView curentPage, totalPage,empt_infos;
	private ListView all_device_list = null;
	private ExpandableListView expListView = null;
	private String selectGroup = null;
	private List<JgdmVO> enterList = null;
	private JgdmListAadpter adapter = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		deviceNumber.setText(StringUtil.convertNull(jgdm));
		codeValue.setText(StringUtil.convertNull(jgmc));
	...
	private void findView() {
		...
		empt_infos = (TextView)findViewById(R.id.empt_infos);
	...
	private void initAdapter() {
		if (enterList != null && enterList.size() > 0) {
			empt_infos.setVisibility(View.GONE);
			curentPage.setText(pageIndex + "/" + page);
			totalPage.setText(pageCount + "条");
			adapter = new JgdmListAadpter(enterList,this);
			all_device_list.setAdapter(adapter);
			adapter.notifyDataSetChanged();
		}else{
			curentPage.setText(0 + "/" + page);
			totalPage.setText(pageCount + "条");
			
			empt_infos.setVisibility(View.VISIBLE);
			empt_infos.setText("没有检索到给定条件的数据列表!");
		}
	}
	...
	private class SearchDeviceAsy extends AsyncTask<String, String, String>
		...
		public void execute(String response, int kind) {
			try {
				...
				enterList = new ArrayList<JgdmVO>();
				for (int i = 0; i < arry.length(); i++) {
					JgdmVO ent = new JgdmVO();
	...
	class SearchDevice implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (!StringUtil.isEmpty(keyword) && keyword.length() != 9){
				Toast.makeText(JgdmListActivity.this, "需输入9位的机构代码!", 3000).show();
				return;
			}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
		JgdmVO ent = (JgdmVO) all_device_list.getAdapter().getItem(position);
		Intent inte = new Intent(this,JgdmInfoActivity.class);
