...
import org.nacao.searchapp.model.Jgdm;
...
public class JgdmInfoActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		jgmc=(String) inte.getSerializableExtra("jgmc");
	...
	private class jgdmInfoAsy extends AsyncTask<String, String, String> implements HttpConnectionCallback{
		...
		protected String doInBackground(String... params) {
			...
			try {
				if(!StringUtil.isEmpty(jgmc)){
					parms.put("jgmc", URLEncoder.encode(jgmc,"GBK"));
				}
		...
		protected void onPostExecute(String result) {
			...
			if (requestCount == 3) {
				requestCount=0;
				Toast.makeText(JgdmInfoActivity.this, "当前网络不好，请检查网络！", 3000).show();
				return;
			}
			if (flag) {
				return;
...
//			System.out.println("result：："+result);
			Log.v("searchapp", "手机号码为"+DisplayTool.getPhoneNum(JgdmInfoActivity.this)+"的用户,"+DateProcess.getSysTime()+"：查看了（"+(!StringUtil.isEmpty(jgdmInfo.getJgdm())?"机构代码："+jgdmInfo.getJgdm():"")+"）机构的详细信息，登录用户为："+Variable.user.getUserName()+"。");
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				if(msg.obj!=null){
					...
					errInfos.setVisibility(View.GONE);
					showInfo();
				}else{
					errInfos.setVisibility(View.VISIBLE);
					errInfos.setText("没有检索到给定条件"+(!StringUtil.isEmpty(codeid)?"(机构代码："+codeid+")":"")+(!StringUtil.isEmpty(jgmc)?"(机构名称："+jgmc+")":"")+"的数据!");
				}
				break;
			default:
				break;
